"""LangChain multi-agent demo with workflow-optimizer-sdk tracing."""
...
from tools import close_client, reset_mcp_state, set_mcp_url

log = get_logger("langchain_multi_agent")
...
async def run_scenario(
    ...
    async with optimizer.trace(
        ...
        agent_name="LangChainMultiAgent",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run LangChain multi-agent demo")
...
async def main() -> None:
    ...
    log.info(
        "multi_agent_demo_start",
        rounds=args.rounds,
        collector=args.collector_url,
        mcp=args.mcp_url,
    )

    await set_mcp_url(args.mcp_url)
    graph = build_multi_agent_graph(args.model)
            ...
            for round_num in range(1, args.rounds + 1):
                ...
                for scenario in SCENARIOS:
                    result = await run_scenario(
                        scenario, round_num, graph, optimizer, args.model,
                    )
