...
async def receive_event(event: EventIn, request: Request) -> dict:
    ...
    await request.app.state.ws_manager.broadcast(event.model_dump(mode="json"))
...
async def receive_batch(batch: BatchEventsIn, request: Request) -> dict:
    ...
    json_events = [e.model_dump(mode="json") for e in batch.events]
    await request.app.state.ws_manager.broadcast_batch(json_events)
